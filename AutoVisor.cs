...
public class AutoVisor : IDalamudPlugin
{
    ...
    private void OnPPose(string command, string arguments)
    {
        ...
        if (whichPose == 0 || whichPose > CPoseManager.Num(whichPoseType))
        {
            Dalamud.Chat.PrintError(
                $"Pose {whichPose} for {PoseTypes[(int)whichPoseType]} does not exist, only {CPoseManager.Num(whichPoseType)} poses are supported.");
            return;
        }

        VisorManager!.CPoseManager.SetPose(whichPoseType, (byte)(whichPose - 1));
        Dalamud.Chat.Print($"Set {PoseTypes[(int)whichPoseType]} pose to {whichPose}.");
    }
}
