...
public class AutoVisorUi
{
    ...
    private static readonly string[] PoseOptions =
        Enumerable.Range(1, 20).Select(x => $"Pose {x}").Prepend("Unchanged").Prepend("Default").ToArray();
    ...
    private static readonly string[] PoseTooltips = CreateTooltips();

    private static string[] CreateTooltips()
    {
        var tooltips = new string[]
        {
            ...
            "Change your default pose when holding an umbrella.\nDefault sets the pose from login with that character.\nUnchanged does not change the pose at all.",
            "Change your default pose when using accessories other than umbrellas.\nDefault sets the pose from login with that character.\nUnchanged does not change the pose at all.",
        };
        return tooltips.Concat(Enumerable.Repeat(string.Empty, Math.Max(0, CPoseManager.PoseNames.Length - tooltips.Length))).ToArray();
    }
    ...
    public void DrawPoseTableContent(PlayerConfig settings, Job job, VisorChangeGroup jobSettings)
    {
        foreach (var pose in Enum.GetValues<PoseType>())
        {
            ...
            if (ImGui.Combo($"##03_{_currentPlayer}_{_currentJob}_pose_{(int)pose}", ref tmp, PoseOptions, CPoseManager.Num(pose) + 2)
             && tmp != copy)
            {
                ...
                jobSettings.SetPose(pose, value);
                settings.PerJob[job] = jobSettings;
                Save();
            }

            if (ImGui.IsItemHovered())
                ImGui.SetTooltip(PoseTooltips[(int)pose]);
        }
    }
